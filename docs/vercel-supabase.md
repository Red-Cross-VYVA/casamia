...
Run `supabase/public-report-rate-limits.sql` before enabling safety or grant report delivery. This provides a persistent, hashed-IP report throttle that works across Vercel instances. Set `PUBLIC_REPORT_RATE_LIMIT_SALT` to a long random server-only value; the internal session secret remains a fallback.
