...
- The Facebook operations screen reports Page `61574255177723`, Graph API `v26.0` and a configured page access token. A controlled Home Safety Review publish attempt on 28 August 2026 reached Meta but was rejected with OAuth code `190`, subcode `463`: the stored Page access token expired on 23 August 2026. No post was created. Replace the production token and repeat the controlled publish test.
...
- **Facebook:** replace the expired production Page access token, then publish one approved starter post and confirm it appears on Page `61574255177723`. The 28 August controlled attempt created no post.
