...
- The Facebook operations screen reports Page `61574255177723`, Graph API `v26.0` and a configured page access token. After replacing the expired token and redeploying on 28 August 2026, a controlled Home Safety Review publish attempt reached Meta but was rejected with OAuth code `200` and HTTP `403`. Protected token diagnostics confirmed that the deployed token belongs to Page `605133552680332`, not configured CasaMia Page `61574255177723`; this identity mismatch produces Meta's `pages_read_engagement` / `pages_manage_posts` authorization error. No post was created. Generate a Page access token for the configured Page and repeat the controlled publish test.
...
- **Facebook:** replace the Page `605133552680332` token with a Page access token for configured Page `61574255177723`, including `pages_read_engagement` and `pages_manage_posts`; then publish one approved starter post and confirm it appears publicly. The 28 August controlled attempts created no post.
