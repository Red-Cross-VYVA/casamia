...
- The Facebook operations screen reports Page `61574255177723`, Graph API `v26.0` and a configured page access token. After replacing the expired token and redeploying on 28 August 2026, a controlled Home Safety Review publish attempt reached Meta but was rejected with OAuth code `200` and HTTP `403`: the new token lacks `pages_read_engagement` and `pages_manage_posts`. No post was created. Regenerate the production token with both permissions and repeat the controlled publish test.
...
- **Facebook:** regenerate the production Page access token with `pages_read_engagement` and `pages_manage_posts`, then publish one approved starter post and confirm it appears on Page `61574255177723`. The 28 August controlled attempts created no post.
