...
import {
  loadVisitAvailability, scheduleVisit, type VisitAppointment, type VisitAvailability,
...
export function VisitScheduler({ language, sessionId }: { language: string; sessionId: string }) {
  ...
  async function confirm() {
    ...
    setMessage('')
    try {
      const result = await scheduleVisit(sessionId, selectedStartAt)
      setAvailability((current) => current ? { ...current, appointment: result.appointment, dates: [] } : current)
    } catch (error) {
  ...
  if (availability?.appointment) return <AppointmentConfirmation appointment={availability.appointment} copy={copy} language={language} />
  ...
  return (
    <section className="mx-auto mt-5 max-w-5xl rounded-lg border border-blue/25 bg-white p-5 shadow-soft md:p-7" aria-labelledby="visit-scheduler-title">
      <div className="flex items-start gap-3">
        ...
        <div><h2 id="visit-scheduler-title" className="font-display text-2xl font-bold text-text-dark">{copy.title}</h2><p className="mt-1 text-sm font-bold text-text-muted">{copy.body}</p></div>
      </div>
      ...
      {message ? <p className="mt-4 text-sm font-bold text-red-700" role="alert">{message}</p> : null}
      {availability?.dates.length ? <div className="mt-6 flex justify-end"><button className="btn btn-green" disabled={!selectedStartAt || submitting} type="button" onClick={() => void confirm()}>{submitting ? <LoaderCircle className="animate-spin" size={18} /> : <Clock3 size={18} />}{copy.confirm}</button></div> : null}
    </section>
  )
}
