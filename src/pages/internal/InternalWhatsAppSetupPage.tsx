    ...
    window.open = function captureMetaOauthPopup(url, target, features) {
      if (url) {
        try {
          ...
          // Meta will surface malformed popup URLs through the login callback.
        }
      }
        ...
        if (code) {
          ...
          if (sessionFinishedRef.current) {
            ...
            if (!oauthRedirectUri) {
              ...
              setResult((current) => ({
                ...current,
                error: 'CasaMia could not capture the callback generated by Meta for this authorization.',
              }))
              setMessage('Authorization was received, but the Meta callback could not be verified.')
              return
            }
            void resolveIdentifiers(code, oauthRedirectUri)
          }
        } else {
          clearTransferTimeout()
          setIsStarting(false)
